...
**failed**: 告诉教练 ❌ 或 ⚠️，根据 results 中的 actual vs expected 指出问题，转述 hints 给出排查方向。引导教练在 wp-admin 后台哪里修正，不要用 wp-cli 帮教练执行操作。如果同一任务已失败 3 次以上，可以给出更直接的引导（如具体的菜单路径和按钮位置）。
